...
>
> **用户实测补充**：「调整部署设置」对话框里实际只暴露 5 个区块——应用部署框架 / 关联
> 云资源 / 镜像版本 / 明文变量 / 密文管理，**没有"启动文件"字段**。ModelScope 对
> Streamlit 空间会自动识别入口（约定 `app.py`，仓库里有 `app.py` 和 `streamlit_app.py`
> 两个等价入口都能被识别）；想 100% 确认可看对话框顶部的「**JSON 配置**」tab，会显示
> 完整的启动命令。
...
> **环境变量放哪个区块**：「调整部署设置」对话框下半部分有 **明文变量** 和 **密文管理**
> 两栏。`DATABASE_URL` 含数据库密码，**强烈建议放「密文管理」**（值运行时加密注入，
> UI 不回显）。点「+ 添加密文变量」→ 名称 `DATABASE_URL` → 值贴完整连接串即可。
