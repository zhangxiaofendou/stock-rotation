...
_RAW_REPORTS = [
    # ---- 电力设备：多家上调，目标价上调潮 ----
    ("801730.SI", "中信证券", None, None, "买入", "增持", 38.0, 32.0, "上调", "2026-07-10",
     "储能与海风装机超预期，龙头订单饱满，盈利拐点确认。", "原材料涨价;海外贸易壁垒", "https://example.com/r/eq1"),
    ("801730.SI", "华泰证券", None, None, "买入", "买入", 40.0, 35.0, "维持", "2026-07-08",
     "装机高景气延续，估值仍处历史低位，维持买入。", "竞争加剧", "https://example.com/r/eq2"),
    ("801730.SI", "国泰君安", None, None, "增持", "增持", 36.5, 33.0, "维持", "2026-06-28",
     "板块估值修复途中，盈利确定性提升。", "产能过剩", "https://example.com/r/eq3"),
    ("801730.SI", "中金公司", "300750.SZ", "宁德时代", "买入", "增持", 320.0, 280.0, "上调", "2026-07-05",
     "电池龙头全球份额回升，海外订单放量。", "汇率波动", "https://example.com/r/eq4"),

    # ---- 计算机：评级上调 + 首次覆盖 ----
    ("801750.SI", "广发证券", None, None, "买入", "中性", 45.0, 38.0, "上调", "2026-07-09",
     "AI 应用落地加速，国产算力需求爆发。", "技术迭代;政策落地不及预期", "https://example.com/r/cs1"),
    ("801750.SI", "招商证券", None, None, "增持", None, 42.0, None, "首次", "2026-07-02",
     "信创与算力双主线，首次覆盖给予增持。", "下游预算收紧", "https://example.com/r/cs2"),
    ("801750.SI", "天风证券", None, None, "买入", "买入", 44.0, 40.0, "维持", "2026-06-26",
     "行业景气上行，龙头业绩兑现。", "估值偏高", "https://example.com/r/cs3"),

    # ---- 医药生物：覆盖券商数量上升 ----
    ("801150.SI", "兴业证券", None, None, "增持", "增持", 30.0, 28.0, "维持", "2026-07-06",
     "创新药出海逻辑强化，政策边际缓和。", "医保控费;临床失败", "https://example.com/r/med1"),
    ("801150.SI", "东吴证券", None, None, "买入", None, 33.0, None, "首次", "2026-07-01",
     "CXO 订单回暖，首次覆盖看多。", "地缘政治", "https://example.com/r/med2"),
    ("801150.SI", "国盛证券", None, None, "增持", "中性", 31.0, 27.0, "上调", "2026-06-24",
     "集采影响出清，板块配置价值凸显。", "集采扩围", "https://example.com/r/med3"),

    # ---- 电子：分化（有上调也有维持） ----
    ("801080.SI", "国信证券", None, None, "买入", "增持", 52.0, 46.0, "上调", "2026-07-07",
     "半导体国产替代加速，设备订单高增。", "周期波动", "https://example.com/r/el1"),
    ("801080.SI", "长江证券", None, None, "增持", "增持", 49.0, 47.0, "维持", "2026-06-30",
     "消费电子温和复苏，等待旺季验证。", "需求疲弱", "https://example.com/r/el2"),

    # ---- 有色金属：下调（对比，形成分歧） ----
    ("801050.SI", "中泰证券", None, None, "中性", "增持", 22.0, 25.0, "下调", "2026-07-04",
     "商品价格回落，盈利预期下修。", "价格继续下行", "https://example.com/r/met1"),
    ("801050.SI", "方正证券", None, None, "减持", "中性", 19.0, 23.0, "下调", "2026-06-27",
     "供需转弱，建议降低配置。", "宏观下行", "https://example.com/r/met2"),

    # ---- 食品饮料：中性（关注度平稳） ----
    ("801120.SI", "海通证券", None, None, "增持", "增持", 88.0, 85.0, "维持", "2026-07-03",
     "消费弱复苏，龙头份额稳固，维持增持。", "需求不及预期", "https://example.com/r/fb1"),
    ("801120.SI", "申万宏源", None, None, "中性", "中性", 80.0, 80.0, "维持", "2026-06-25",
     "板块缺乏催化，等待基本面信号。", "成本上行", "https://example.com/r/fb2"),

    # ---- 汽车：首次覆盖 ----
    ("801880.SI", "东方证券", None, None, "买入", None, 28.0, None, "首次", "2026-07-08",
     "智能驾驶渗透率提升，整车出海提速，首次覆盖买入。", "价格战", "https://example.com/r/auto1"),

    # ---- 国防军工：上调 ----
    ("801740.SI", "安信证券", None, None, "买入", "增持", 60.0, 54.0, "上调", "2026-07-06",
     "装备列装加速，订单确定性高。", "交付节奏", "https://example.com/r/mil1"),
]

# 新闻示例：(sector_code, headline, category, sentiment, published_at, source, url)
_RAW_NEWS = [
    ("801730.SI", "国家能源局：上半年新型储能装机同比增长超 80%", "政策", "positive", "2026-07-12", "财联社", "https://example.com/n1"),
    ("801750.SI", "国产大模型再获重大升级，多家厂商宣布开放API", "行业", "positive", "2026-07-11", "证券时报", "https://example.com/n2"),
    ("801050.SI", "伦敦金属交易所铜价单周下跌 4%，有色板块承压", "行业", "negative", "2026-07-10", "华尔街见闻", "https://example.com/n3"),
    ("801150.SI", "多款国产创新药获FDA突破性疗法认定", "行业", "positive", "2026-07-09", "医药魔方", "https://example.com/n4"),
    ("801880.SI", "6月新能源汽车零售渗透率突破 50%", "行业", "positive", "2026-07-08", "乘联会", "https://example.com/n5"),
    ("801120.SI", "部分白酒渠道库存高企，旺季动销承压", "行业", "negative", "2026-07-07", "21世纪经济报道", "https://example.com/n6"),
]
